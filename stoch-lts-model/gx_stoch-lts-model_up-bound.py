...
upper_bound = 3.95*df["X_0"]
upper_bound_2 = 5*df["X_0"]

yva[yva>upper_bound]=upper_bound
...
yvals = df["wrecs"][i][k:low]
yvals[yvals>upper_bound] = upper_bound
...
ax.plot(range(k,df["Tmax"]-125),df["wrecs"][i][l:l+df["Tmax"]-k-125], 'grey')

ax.plot([k, df["Tmax"]-125*1.05],[df["XT"]]*2, 'black', linestyle=':')


ax.plot([xlow, df["Tmax"]-125*1.05],[upper_bound]*2, 'black', linestyle=':')
ax.plot([xlow, df["Tmax"]-125*1.05],[upper_bound_2]*2, 'black', linestyle=':')
...
ax.set_xlim(xlow, df["Tmax"]-125*1.05)
ax.set_ylim(bottom=-0.0875)


pl.xticks([z-(df["Tmax"]-900)+22,z,k, df["Tmax"]-125], ['$t=0$', '','', '$t=t_{\mathrm{max}}$'])
